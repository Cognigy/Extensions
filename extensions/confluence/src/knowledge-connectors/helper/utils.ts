...
export const getPages = async (
	...
	if (!pageId && !folderId)
		throw new Error(
			"Invalid Confluence URL: Must contain either a page ID or folder ID",
		);
...
export const getPageChunks = async (
	...
	try {
		...
		const parser = new ConfluenceDataParser(
			xhtml,
			sourceName,
		...
		throw new Error(
			`Error processing Confluence source, with page Id: ${pageId}: ${error}`,
		);
	}
	return result;
};
