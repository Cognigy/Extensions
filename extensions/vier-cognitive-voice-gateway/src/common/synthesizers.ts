...
export function synthesizersFields(extraCondition?: TNodeFieldCondition): INodeField[] {
    ...
    const vendorSelectPrimary: INodeField = makeVendorSelect(
        "SynthesizerVendorSelectPrimary",
        t.textToSpeech.inputServiceLabel,
        t.textToSpeech.inputServiceDescription,
        true,
    )

    const vendorSelectFallback: INodeField = makeVendorSelect(
        "SynthesizerVendorSelectFallback",
        t.textToSpeech.inputServiceFallbackLabel,
        t.textToSpeech.inputServiceFallbackDescription,
        false,
    )
    ...
    return [
        ...
        },
        {
            ...
            condition: {
                and: [
                    extraCondition,
                    { key: vendorSelectPrimary.key, value: "PROFILE_TOKEN", negate: true },
                ],
            },
        },
        ...
        },
        {
            ...
            condition: {
                and: [
                    extraCondition,
                    { key: vendorSelectFallback.key, value: "PROFILE_TOKEN", negate: true },
                    { key: vendorSelectFallback.key, value: "", negate: true },
                ],
            },
        },
    ]
...
function convertVendorSelect(
    vendorSelectValue: string,
    profileTokenValue: string,
    voiceValue: string,
): any | undefined {
...
export function convertSynthesizers(inputs: SynthesizersInputs): Array<any> {
    ...
    const fallback = convertVendorSelect(
        ...
        inputs.SynthesizerProfileTokenFallback,
        inputs.SynthesizerVoiceFallback,
    )
...
export function convertSynthesizersIfChanged(
    inputs: SynthesizersInputsWithToggleToUseDefault,
): Array<any> | null {
...
export const synthesizersWithToggleToUseDefaultFieldKeys: readonly string[] =
    synthesizersFieldWithToggleToUseDefault().map(field => field.key)
