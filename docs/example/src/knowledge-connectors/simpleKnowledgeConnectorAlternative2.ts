...
export const simpleKnowledgeConnector = createKnowledgeConnector({
    ...
    for (const name of exampleSources) {
      ...
      // Additional check if the content for the given source is up to date
      if (!api.isSourceUpToDate({
        // Only name is used as the identifier because there is a unique constraint on the DB level.
        // We could also add a new field "externalId" to allow multiple source with the same name and
        ...
        for (const text of values) {
          // In case the content is up to date he `api.createKnowledgeChunk` is a NOOP and does not insert new chunks
