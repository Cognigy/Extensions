...
export interface IGetEventsFromCalendarParams extends INodeFunctionBaseParams {
	config: {
		accessToken: string;
		userSource: string;
		userMail?: string;
...
export const getEventsFromCalendarNode = createNodeDescriptor({
	type: "getEventsFromCalendar",
	defaultLabel: "Get Events From Calendar",
	preview: {
		key: "userSource",
		type: "text"
	},
	fields: [
		...
		},
		{
			...
			defaultValue: "microsoft.calendar",
			condition: {
				key: "storeLocation",
				value: "input",
			}
	...
	function: async ({ cognigy, config }: IGetEventsFromCalendarParams) => {
		const { api } = cognigy;
