...
export interface IAssignTaskParams extends INodeFunctionBaseParams {
	config: {
		authType: string;
		instanceInfo: {
...
export const assignTaskNode = createNodeDescriptor({
	...
	fields: [
		...
		},
		{
			...
			defaultValue: "uipath.assignedTask",
			condition: {
				key: "storeLocation",
				value: "input"
			}
	...
	function: async ({ cognigy, config }: IAssignTaskParams) => {
		const { api } = cognigy;
		...
		if (authType === 'cloud') {
			...
			endpoint = `https://platform.uipath.com/${accountLogicalName}/${tenantLogicalName}/odata/Tasks/UiPath.Server.Configuration.OData.AssignTasks`;
		} else { // onPrem
			const { orchestratorUrl } = onPremAuthConnection;
			endpoint = `https://${orchestratorUrl}/odata/Tasks/UiPath.Server.Configuration.OData.AssignTasks`;
		}
