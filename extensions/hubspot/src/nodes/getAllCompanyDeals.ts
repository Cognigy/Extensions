...
export interface IGetAllCompanyDealsParams extends INodeFunctionBaseParams {
	config: {
		connection: {
			accessToken: string;
		};
...
export const getAllCompanyDealsNode = createNodeDescriptor({
	type: "getAllCompanyDeals",
	defaultLabel: "Get Company Deals",
	fields: [
		{
			key: "connection",
	...
	function: async ({ cognigy, config }: IGetAllCompanyDealsParams) => {
		const { api } = cognigy;
		const {
			storeLocation,
			contextKey,
 ...
 */
async function getCompanyDeals(accessToken: string, objectId: number): Promise<any> {
	...
	try {
		...
		try {
			const response = await hubspot.deals.getAssociated('COMPANY', objectId, {properties: ['dealname']});
