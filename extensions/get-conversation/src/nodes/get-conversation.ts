...
export async function getConversationFunction({ cognigy, config }: IGetConversationParams) {
	...
	// if (!functionName) throw new Error('aws.GetConversation: The Function Name is missing.');

	// const { region, accessKeyId, secretAccessKey } = connection;
	// if (!region) throw new Error(`aws.GetConversation: AWS 'region' is missing from the connection data.`);
	// if (!accessKeyId) throw new Error(`aws.GetConversation: AWS 'accessKeyId' is missing from the connection data.`);
	// if (!secretAccessKey) throw new Error(`aws.GetConversation: AWS 'secretAccessKey' is missing from the connection data.`);
	...
	if (config.storeLocation === 'context') {
		cognigy.api.setContext(config.contextKey, returnVal);
	} else {
		cognigy.input[config.inputKey] = returnVal;
	}
