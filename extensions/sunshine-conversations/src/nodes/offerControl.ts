...
export const offerControlNode = createNodeDescriptor({
	type: "offerControl",
	defaultLabel: "Offer Control",
	fields: [
		{
			key: "connection",
	...
	function: async ({ cognigy, config }: IOfferControlParams) => {
		...
		try {
			...
			const response = await axios({
				method: 'post',
				url: `https://api.smooch.io/v2/apps/${appId}/conversations/${conversationId}/offerControl`,
				auth: {
					username: keyId,
					password: secret
				},
